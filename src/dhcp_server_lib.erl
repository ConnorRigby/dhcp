...
encode(D) when is_record(D, dhcp_msg) ->
    Op      = D#dhcp_msg.op,
    Htype   = D#dhcp_msg.htype,
    Hlen    = D#dhcp_msg.hlen,
    Hops    = D#dhcp_msg.hops,
    Xid     = D#dhcp_msg.xid,
    Secs    = D#dhcp_msg.secs,
    Flags   = D#dhcp_msg.flags,
    Ciaddr  = ip_to_binary(D#dhcp_msg.ciaddr),
    Yiaddr  = ip_to_binary(D#dhcp_msg.yiaddr),
    Siaddr  = ip_to_binary(D#dhcp_msg.siaddr),
    Giaddr  = ip_to_binary(D#dhcp_msg.giaddr),
    Chaddr  = pad(eth_to_binary(D#dhcp_msg.chaddr), 16),
    Sname   = pad(list_to_binary(D#dhcp_msg.sname), 64),
    File    = pad(list_to_binary(D#dhcp_msg.file), 128),
    Opts    = options_to_binary(D#dhcp_msg.options),
